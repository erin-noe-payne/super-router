...
Router.prototype.route = function route(path, method, headers, inputStream, done) {
  ...
  var responseStream = new SuperRouterStream();
  if(true){//TODO: check to see if we need to buffer
    var buffer = '';
    ...
    inputStream.on('end', function (){
      var body = {};
      if(buffer != ''){
        if(true){//TODO: check to see if we need to deserialize with protofile
  ...
  // if(false && headers.isStream){
  //   inputStream.pipe(requestStream);
  //   matchedRoute.handler(requestStream, reponseStream);
  // }
  // else {
  //   var buffer = '',
  //     body;
  ...
  //     //   body = JSON.parse(buffer)
  //     // }
  //     // if(headers.contentType == 'application/proto') {
  //     //   body = protoMessage.decode(buffer).toJSON();
  //     // }
  //     requestStream.body = JSON.parse(buffer);
  //     matchedRoute.handler(requestStream, reponseStream);
  //   });
  // }
