...
Router.prototype.route = function route(path, method, headers, inputStream, done) {
  ...
  if(inputStream instanceof SuperRouterStream){
    return self._sendToHandler(inputStream, responseStream, matchedRoute, done);
  }
  ...
  if(_doWeNeedToBufferRequest(headers)){//check to see if we need to buffer
    ...
    inputStream.on('end', function (){
      ...
      requestStream.routeInfo = {};
      _.each(input, function (value, key) {
        if(key[0] == '_'){
          requestStream.routeInfo[key] = value;
          delete input[key];
        }
      });
      ...
      self._sendToHandler(requestStream, responseStream, matchedRoute, done);
    });
