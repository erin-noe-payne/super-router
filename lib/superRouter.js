...
Router.prototype.route = function route(path, method, headers, input, done) {
  ...
  matchedRoute = self._matchPathAndMethod(path, method)

  //if we didn't get a match, we need to send down a 405 or a 404
  if(_.isUndefined(matchedRoute)) { return self._noMatch(path, done); }
  logger.debug('Request:\n', {path: matchedRoute.path, method: matchedRoute.method, trackingGUID: trackingGUID, inputProto: matchedRoute.inputProto, input: input});


  //merge input from querystring in URI and return 400 if there's a missmatch
  input = _mergeInputs(input, matchedRoute.route.match(path));
  if(_.isError(input)){
  ...
  var responseStream = new SuperRouterStream()
  matchedRoute.handler(headers, input, responseStream);
  ...
  //Logging here causes tests to fail, not sure why
  // responseStream.pipe(through2.obj(function(chunk, enc, callback){
  //   logger.debug('Response:\n', {path: matchedRoute.path, method: matchedRoute.method, trackingGUID: trackingGUID, outputProto: matchedRoute.outputProto, output: chunk});
  // }));
  done(responseStream);
}
