...
class App {
   ...
   *
   * If a handler throws an error or rejects its promise, it will go to the error handler supplied. If
   * no error handler was supplied it will be caught by the next registered catch in the stack.
   *
   * @example
   * app.then({
   *  path : '/a/b',
  ...
  then(route) {
    ...
    this._middleware.push({ route, errorMiddleware : false });
    return this;
  }
   ...
   */
    ...
    this._middleware.push({ route, errorMiddleware : true });
    return this;
  }
  ...
  processRequest(request) {
    ...
    _.each(this._middleware, (registeredRoute) => {
      const route = registeredRoute.route;
      const errorMiddleware = registeredRoute.errorMiddleware;
      if (!errorMiddleware) {
