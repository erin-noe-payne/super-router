...
class Route {
   ...
   *  matches all paths.
   * @param {String} [method='*'] - The method that the route should be executed against. If not provided, mathes
   *  all methods.
  ...
  constructor(options) {
    ...
    if (options.method && !utils.isValidMethodArray(options.method)) {
    ...
    this._methods = this._convertAndNormalizeMethods(options.method || METHODS.ALL);
  ...
  _isMethodMatch(method) {
    return (_.contains(this.method, METHODS.ALL)) || (_.difference(this._convertAndNormalizeMethods(method), this.method).length === 0);
  }
