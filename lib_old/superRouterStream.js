...
var RouterError = builder.build("SuperRouter").Error;
...
SuperRouterStream.prototype.error = function(id, code, message, prev_error){
  if(_.isUndefined(id) || _.isUndefined(code) || _.isUndefined(message)){
    throw new Error("id, code, and message must be defined to send an error");
  }

  if(!_.isString(message)){
    throw new Error("id, code, and message must be strings");
  }

  var body = new RouterError({
    id: id,
    code: code,
    message: message,
